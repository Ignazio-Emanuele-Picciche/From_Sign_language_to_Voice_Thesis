...
from src.models.seven_classes.vivit.video_dataset import VideoDataset  # Aggiunto import
from src.models.seven_classes.lstm_model import EmotionLSTM
from src.models.seven_classes.stgcn_model import STGCN


def get_data_paths(base_dir):
    """Restituisce i percorsi standard per i dati di training e validazione per 7 classi."""
    train_landmarks_dirs = [
        ...
        os.path.join(
            ...
            "raw",
            "ASLLRP",
            "mediapipe_output_with_neutral_0.34",
            "json",
        ),
    ]
    train_processed_files = [
        os.path.join(
            ...
            "processed",
            "train",
            "video_sentiment_data_7_classes.csv",  # TODO: Creare questo file per How2Sign
        ),
        os.path.join(
            base_dir,
            "data",
            "processed",
            "asllrp_video_sentiment_data_7_classes_without_golden.csv",  # File a 7 classi
        ),
    ]
    ...
    val_processed_file = os.path.join(
        ...
        "processed",
        "val",
        "video_sentiment_data_7_classes.csv",  # TODO: Creare questo file per validazione
    )
    return (
        train_landmarks_dirs,
        train_processed_files,
...
def get_datasets(
    ...
    # Passa le etichette del training set al validation set per garantire coerenza
    val_dataset = LandmarkDataset(
        landmarks_dir=val_landmarks_dir,
        processed_file=val_processed_file,
        labels=train_dataset.labels,
    )

    if downsample_majority_class:
        print("Esecuzione del downsampling sulla classe maggioritaria...")
        # ! VECCHIA LOGICA (ERRATA PER 3 CLASSI): Assumeva solo 'Positive' e 'Negative'
        ...
        # ! NUOVA LOGICA (DOWNSAMPLING SELETTIVO SOLO PER CLASSE NEUTRAL)
        # Per il problema 7-classi con sbilanciamento estremo (Neutral 50%, Extremely Negative 0.5%),
        # fare downsampling di TUTTE le classi verso la minoritaria distrugge i dati.
        # SOLUZIONE: Downsample SOLO Neutral verso la seconda classe più frequente (es. Positive).

        df_train = train_dataset.processed
        emotion_counts = df_train["emotion"].value_counts()

        # Trova la classe più frequente (dovrebbe essere Neutral con ~2049 samples)
        most_frequent_class = emotion_counts.idxmax()
        most_frequent_count = emotion_counts.max()

        # Trova la seconda classe più frequente come target
        second_most_frequent_count = emotion_counts.nlargest(2).iloc[1]

        # Calcola quanti samples tenere per la classe più frequente
        # downsample_ratio = 0.4 significa: ridurla al 40% della seconda classe più frequente
        target_count_for_majority = int(second_most_frequent_count * downsample_ratio)

        downsampled_dfs = []

        # Itera su ogni gruppo di emozioni
        for emotion, group in df_train.groupby("emotion"):
            if (
                emotion == most_frequent_class
                and len(group) > target_count_for_majority
            ):
                # Downsample SOLO la classe più frequente (Neutral)
                downsampled_group = group.sample(
                    n=target_count_for_majority,
                    random_state=42,
                )
                downsampled_dfs.append(downsampled_group)
                print(
                    f"  Downsampling '{emotion}': {len(group)} → {target_count_for_majority}"
                )
            else:
                # Mantieni TUTTE le altre classi intatte
                downsampled_dfs.append(group)
                print(f"  Mantenendo '{emotion}': {len(group)} samples")

        df_train_downsampled = pd.concat(downsampled_dfs)
        ...
        print(f"Train set originale: {emotion_counts.to_dict()}")
        print(
...
def get_video_datasets(
    ...
    if downsample_majority_class:
        print("Esecuzione del downsampling selettivo per la classe Neutral...")
        df_train = train_dataset.video_info
        class_counts_train = df_train["emotion"].value_counts()

        # Trova la classe più frequente (dovrebbe essere Neutral)
        most_frequent_class = class_counts_train.idxmax()
        most_frequent_count = class_counts_train.max()

        # Trova la seconda classe più frequente come target
        second_most_frequent_count = class_counts_train.nlargest(2).iloc[1]
        ...
        for emotion, group in df_train.groupby("emotion"):
            if (
                ...
                downsampled_dfs.append(
                    group.sample(n=target_count_for_majority, random_state=42)
                )
                print(
                    f"  Downsampling '{emotion}': {len(group)} → {target_count_for_majority}"
                )
...
def setup_ignite_evaluator(model, criterion, device, model_type, val_loader):
    ...
    def compute_f1_scores(engine):
        ...
        # ) # Vecchio codice per 2 classi
        engine.state.metrics["val_f1_class_0"] = (
            float(f1_per_class[0]) if len(f1_per_class) > 0 else 0.0
        )
        engine.state.metrics["val_f1_class_1"] = (
            float(f1_per_class[1]) if len(f1_per_class) > 1 else 0.0
        )
        engine.state.metrics["val_f1_class_2"] = (
            float(f1_per_class[2]) if len(f1_per_class) > 2 else 0.0
        )  # Aggiunto per la terza classe
