...
def get_data_paths(base_dir):
    ...
    train_processed_files = [
        ...
        os.path.join(
            ...
            "data",
            "processed",
            "asllrp_video_sentiment_data_with_neutral_0.34_without_golden.csv",  # Nuovo path
        ),
    ]
...
def get_datasets(
    ...
    if downsample_majority_class:
        ...
        # ! NUOVA LOGICA (CORRETTA PER MULTI-CLASSE)
        df_train = train_dataset.processed
        emotion_counts = df_train["emotion"].value_counts()
        minority_class_name = emotion_counts.idxmin()
        n_minority = emotion_counts.min()

        downsampled_dfs = []

        # Itera su ogni gruppo di emozioni
        for emotion, group in df_train.groupby("emotion"):
            if emotion == minority_class_name:
                # Mantieni tutti i campioni della classe minoritaria
                downsampled_dfs.append(group)
            else:
                # Sottocampiona le altre classi
                n_samples_to_keep = int(n_minority * downsample_ratio)
                downsampled_group = group.sample(
                    n=min(
                        len(group), n_samples_to_keep
                    ),  # Assicura di non chiedere più campioni di quelli disponibili
                    random_state=42,
                )
...
def get_video_datasets(
    ...
    if downsample_majority_class:
        print("Esecuzione del downsampling per bilanciare le classi...")
        df_train = train_dataset.video_info
        class_counts_train = df_train["emotion"].value_counts()
        minority_class_label = class_counts_train.idxmin()
        minority_count = class_counts_train.min()

        n_samples_per_class = int(minority_count * downsample_ratio)

        downsampled_dfs = []
        for emotion, group in df_train.groupby("emotion"):
            if emotion == minority_class_label:
                ...
                downsampled_dfs.append(
                    group.sample(
                        n=min(len(group), n_samples_per_class), random_state=42
                    )
                )
