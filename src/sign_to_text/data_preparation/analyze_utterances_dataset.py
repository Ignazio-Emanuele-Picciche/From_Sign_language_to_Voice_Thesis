...
Usage:
    python src/sign_to_text/data_preparation/analyze_utterances_dataset.py
"""
...
def analyze_utterances_dataset(
    ...
    # 6. Train/Val Split (Test = golden_label_sentiment.csv)
    print(f"\n6️⃣  Creazione split train/val...")

    # Shuffle e split
    df_shuffled = df_available.sample(frac=1, random_state=42).reset_index(drop=True)

    n_total = len(df_shuffled)
    n_train = int(0.85 * n_total)  # 85% train
    n_val = n_total - n_train  # 15% val
    ...
    print(f"      - Test:  golden_label_sentiment.csv (176 video con emotion labels)")
    ...
    stats = {
        ...
        "test_dataset": "golden_label_sentiment.csv (176 video)",
    }
    ...
    print(f"   ✓ Splits salvati: {output_dir}/train_split.csv, val_split.csv")
    print(f"   ℹ️  Test set: data/processed/golden_label_sentiment.csv")
    ...
    axes[1, 1].set_title("Train/Val Split (Test = golden_label_sentiment.csv)")
    ...
    return df_available, train_df, val_df, stats
