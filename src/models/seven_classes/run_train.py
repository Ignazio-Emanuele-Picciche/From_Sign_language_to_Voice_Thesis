# =================================================================================================
# COMANDI UTILI PER L'ESECUZIONE - 7 CLASSI
# =================================================================================================
#
...
#
# 2. ESEGUIRE L'ADDESTRAMENTO DEL MODELLO (7 CLASSI)
#    Questo script permette di addestrare un modello per il riconoscimento delle emozioni
#    con granularità a 7 classi: Extremely Negative, Negative, Somewhat Negative, Neutral,
#    Somewhat Positive, Positive, Extremely Positive
#
...
def main(args):
    ...
    base_experiment_name = "7 Classes - Emotion Recognition"
    ...
    if experiment and experiment.lifecycle_stage == "deleted":
        print(
            f"L'esperimento '{experiment_name}' è stato eliminato. Ne creo uno nuovo con un timestamp."
        )
    ...
    with mlflow.start_run(run_name=run_name) as run:
        params = {
            ...
            "patience": args.patience,
            "seed": args.seed,
            "num_classes": 7,  # 7 classi per questo sistema
            "loss_function": "FocalLoss",  # Usando Focal Loss per gestire class imbalance
            "focal_alpha": 1.9548647782429915,
            "focal_gamma": 2.5811066020010545,
        ...
        def validate_and_log(engine):
            ...
            with torch.no_grad():
                ...
                for xb, yb in val_loader:
                    ...
                    all_probs.extend(probs.cpu().numpy())  # Salva tutte le probabilità
                ...
                # Analisi per classe (ora per 3 classi)
                all_probs = np.array(all_probs)
                avg_probs_per_class = []
                for i in range(num_classes):
                    # Calcola la probabilità media predetta per i campioni che appartengono realmente alla classe i
                    class_probs = all_probs[np.array(all_labels) == i]
                    avg_prob = (
                        np.mean(class_probs, axis=0)
                        if len(class_probs) > 0
                        else np.zeros(num_classes)
                    )
                    avg_probs_per_class.append(avg_prob)

                # Vecchia analisi per 2 classi
            ...
            # Aggiungi F1 e probabilità per ogni classe
            for i in range(num_classes):
                metrics_to_log[f"val_f1_class_{i}"] = metrics.get(
                    f"val_f1_class_{i}", 0
                )
                # Logga la probabilità media predetta per la classe i, quando il campione è della classe i
                metrics_to_log[f"avg_prob_for_true_class_{i}"] = avg_probs_per_class[i][
                    i
                ]
            ...
            if engine.state.epoch % 10 == 0:
                ...
                for i in range(num_classes):
                    print(
                        f"Avg prob for true class '{train_dataset.labels[i]}': {avg_probs_per_class[i][i]:.4f}"
                    )
                ...
                print(
                    classification_report(
                        all_labels, all_preds, target_names=train_dataset.labels
                    )
                )
