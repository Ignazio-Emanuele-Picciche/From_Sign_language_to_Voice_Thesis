# =================================================================================================
# TEST SCRIPT PER GOLDEN LABELS A 7 CLASSI
# =================================================================================================
#
# Questo script testa i modelli addestrati con 7 classi sui golden labels.
#
# IMPORTANTE: Le golden labels contengono solo 5 delle 7 classi:
# - Extremely Negative, Negative, Somewhat Negative, Positive, Extremely Positive
# - MANCANO: Neutral, Somewhat Positive
#
...
from src.utils.seven_classes.training_utils import prepare_batch
from src.models.seven_classes.fix_golden_labels_normalization import (
    FixedGoldenLabelDataset,
)

# Import the LSTM model class so MLflow can unpickle it
from src.models.seven_classes import lstm_model

# Create an alias for backward compatibility with old saved models
sys.modules["src.models.seven_classes.lstm_model"] = lstm_model
...
def print_detailed_results_fixed(y_true, y_pred, y_probs, labels):
    ...
    for i in range(min(10, len(y_true))):
        ...
        print(
            f"  {i+1:2d}: True={true_label:<8} Pred={pred_label:<8} {probs_str} Conf={confidence:.3f} {status}"
        )
    ...
    print("=" * 50)
    print("Range delle probabilità per classe:")
    for i, label in enumerate(labels):
        print(f"  {label}: [{y_probs[:, i].min():.3f}-{y_probs[:, i].max():.3f}]")

    print(f"\nPredizioni per classe:")
    for i, label in enumerate(labels):
        count = np.sum(y_pred == i)
        percentage = count / len(y_pred) * 100
        print(f"  {label}: {count} su {len(y_pred)} ({percentage:.1f}%)")
...
def main(args):
    ...
    if args.save_results:
        ...
        output_file = os.path.join(
            BASE_DIR, "results", "golden_labels_test_results_fixed_with_neutral.csv"
        )
        ...
        metrics_file = os.path.join(
            BASE_DIR, "results", "golden_labels_metrics_summary_fixed_with_neutral.csv"
        )
